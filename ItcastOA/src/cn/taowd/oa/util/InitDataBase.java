...
public class InitDataBase {
	...
	public void install() {
		...
		User user = (User) session.createQuery(//
				...
				.setParameter(0, "admin")// 设置参数
				.setParameter(1, MD5Util.encrypt("admin"))// 设置参数
		...
		if (user == null) {
			...
			user2.setName("超级管理员");
			user2.setPassword(MD5Util.encrypt("admin"));
			session.save(user2);// 保存数据
		}
	...
	public static void main(String[] args) {
		System.out.println("----->初始化数据26<--------");
		ApplicationContext ac = new ClassPathXmlApplicationContext("applicationContext.xml");
		InitDataBase init = (InitDataBase) ac.getBean("initDataBase");
		init.install();
	}
